...
while True:
    ...
    start = input(f"{rock} {paper} {scissors} Pick one We'll see who wins to see the scores, please type score, to quit type quit:\n")
    ...
    if random.choice(child_game) == rock and start == paper:
        print("Aw, man, the paper sucumbs the rock... You win! That's a point for you! ")
        your_points += 1
    ...
    if random.choice(child_game) == paper and start == rock:
        ...
        continue
    if start is not rock or paper or scissors:
